...
class MCox_Lasso
{
  ...
  double get_residual_lasso(const MapMatd &alpha, const MapMatd &Gamma, bool get_val = false){
    Z.noalias()   = X * alpha;
...
public:
  MCox_Lasso(int N, int K, int p, int R,
  ...
  double get_gradients(const double *alpha_ptr, const double *Gamma_ptr,
    ...
    double cox_val = get_residual_lasso(alpha, Gamma, get_val);
...
// ============================================================
void prox_lasso(MatrixXd &M_out,
                const MatrixXd &M_in,
                double step_size,
                double lambda)
{
...
//' @param X Covariate matrix (N x p)
 ...
 //' @param lambda_gamma_all Penalty parameter sequence for Gamma (same length as lambda_alpha_all)
 ...
 //' @export
 // [[Rcpp::export]]
 Rcpp::List fit_reduced_rank_lasso(Rcpp::NumericMatrix X,
     ...
     for(int lam_ind = 0; lam_ind < nlambda; ++lam_ind){
       ...
       for(int iter = 0; iter < niter; ++iter){
         ...
         for(int ls_iter = 0; ls_iter < 20; ++ls_iter){
           ...
           prox_lasso(alpha, alpha_temp, current_step, lambda_alpha);
           
           // Gamma: lasso proximal step (no orthogonality constraint)
           prox_lasso(Gamma, Gamma_temp, current_step, lambda_gamma);
...
//' @export
 // [[Rcpp::export]]
 MatrixXd compute_residual_lasso(Rcpp::NumericMatrix X,
                                 Rcpp::NumericMatrix status,
                                 Rcpp::IntegerMatrix rankmin,
                                 Rcpp::IntegerMatrix rankmax,
                                 Rcpp::List order_list,
