...
public:
  MCox_ReducedRank(int N, int K, int p, int R,
  ...
  double get_gradients(const double *alpha_ptr, const double *Gamma_ptr,
  ...
  MatrixXd get_residual_matrix(){ return residual; }
};
...
// ============================================================
void prox_group_lasso_alpha(MatrixXd &M_out,
                            const MatrixXd &M_in,
                            double step_size,
                            double lambda,
                            const std::vector<std::vector<int>> &groups)
{
  ...
  for(int g = 0; g < num_groups; ++g){
    ...
    for(int r = 0; r < R; ++r){
      double norm_sq = 0.0;
      for(int i : idx) norm_sq += M_in(i,r) * M_in(i,r);
      double norm = std::sqrt(norm_sq);
      
      if(norm <= threshold){
        for(int i : idx) M_out(i,r) = 0.0;
      } else {
        double scale = 1.0 - threshold / norm;
        for(int i : idx) M_out(i,r) = M_in(i,r) * scale;
      }
    }
  }
}

// Build group index structure from label vector
...
// Reference: Absil, Mahony & Sepulchre (2008)
// ============================================================
MatrixXd riemannian_gradient(const MatrixXd &grad, const MatrixXd &Gamma)
{
  return grad - Gamma * (grad.transpose() * Gamma);
}


// ============================================================
// Main fitting function (Group Lasso)
// ============================================================
   ...
   if(verbose)
     ...
     for(int lam_ind = 0; lam_ind < nlambda; ++lam_ind){
       ...
       for(int iter = 0; iter < niter; ++iter){
         ...
         current_step = step_size;   // reset step size each iteration
         alpha_prev = alpha;
         Gamma_prev = Gamma;
         ...
         for(int ls_iter = 0; ls_iter < 20; ++ls_iter){
           ...
           prox_group_lasso_alpha(alpha, alpha_temp, current_step, lambda_alpha, groups);
           ...
           cox_val_new = prob.get_value_only(alpha.data(), Gamma.data());
         ...
         for(int g = 0; g < num_groups; ++g){
           ...
           for(int r = 0; r < R; ++r){
             double norm_sq = 0.0;
             for(int i : idx) norm_sq += alpha(i,r) * alpha(i,r);
             penalty_alpha += lambda_alpha * std::sqrt(norm_sq);
           }
         }
...
//' @export
                              ...
                              MatrixXd Gamma)
 {
   ...
   prob.get_gradients(alpha.data(), Gamma.data(),
...
// ============================================================
// Compute dual norm (for lambda_max selection)
// ============================================================
//' @export
 // [[Rcpp::export]]
 VectorXd compute_dual_norm(MatrixXd grad, double alpha, double tol)
 {
   int p = grad.rows();
   VectorXd upperbound =
     (grad.cwiseAbs().rowwise().maxCoeff()).cwiseMin(grad.rowwise().norm() / alpha);
   VectorXd dual_norm(p);
   
   for(int i = 0; i < p; ++i){
     double lower = 0.0, upper = upperbound[i];
     if(upper <= tol){ dual_norm[i] = 0.0; continue; }
     int num_iter = (int)std::ceil(std::log2(upper / tol));
     for(int j = 0; j < num_iter; ++j){
       double bound = (lower + upper) / 2;
       bool less = ((grad.row(i).array().abs() - bound).max(0).matrix().norm())
         <= alpha * bound;
       if(less) upper = bound; else lower = bound;
     }
