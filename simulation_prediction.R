...
## Function to anchor per-outcome sign of B_hat using TRAINING data only.
...
evaluate_one_model_simple <- function(sim_data_train,
  ...
  fit$B_hat <- orient_B_by_training(
    fit$B_hat, X_train,
    y_tr = y_train, s_tr = s_train, e_tr = NULL)
...
## Guarded so that sourcing this file from another script (e.g.
## NEO_data_analysis.R) does NOT trigger the simulation -- it only runs when
## the required objects (search_data_1, sim_data1, auto_*_1, ...) are
## already in the global environment.
if (exists("auto_grplasso_1") && exists("sim_data1") &&
    exists("search_data_1")) {
